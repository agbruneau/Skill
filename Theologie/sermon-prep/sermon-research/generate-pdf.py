...
def generate_markdown(data, output_path):
    ...
    lines.append(f"# Recherche exégétique : {passage}".rstrip())
    lines.append("")
...
def generate_pdf(json_path, output_path=None):
    ...
    if not output_path:
        ...
        output_path = f"Recherche-Exegetique-{safe_name}.pdf"

    doc = create_doc(
        output_path,
        title=f"Recherche exégétique : {data.get('passage', '')}",
    ...
    add_title_banner(story, "RECHERCHE EXÉGÉTIQUE", data.get("passage", ""), meta_parts, styles)
