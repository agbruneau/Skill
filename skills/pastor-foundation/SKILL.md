...
description: Voice, theological guardrails, church context variables (CHURCH_NAME, BIBLE_TRANSLATION, etc.) and output standards shared by every pastor/church skill. ALWAYS read this first when running any church-facing task — sermon-research, sermon-brainstorm, sermon-series, sermon-to-blog, sermon-to-youtube, church-email, church-letter, church-social-post, announcement-script, midweek-devotional, small-group-questions, social-media-calendar, meeting-agenda — or when writing anything addressed to a congregation. Never use it on its own for non-church work.
---
